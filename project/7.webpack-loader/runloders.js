...
function runLoaders(options, finalCallback) {
    ...
    function iterateNormalLoaders(loaderContext, args, finalCallback) {
        ...
        //刚刚raw处理那里，如果是raw，但传进来的已经是buffer的话，走else的话不就变成字符串了吗？
        args = convertArgs(args, normalFn.raw);
        runSyncOrAsync(normalFn, loaderContext, args, finalCallback);
        function runSyncOrAsync(fn, loaderContext, args, callback) {
            ...
            args = fn.call(loaderContext, args);
            if (isSync) {//如果是同步的话。
                loaderContext.loaderIndex--;
                iterateNormalLoaders(loaderContext, args, finalCallback);
            }
...
// 流程：1、各种配置参数（上下文loaderContext）2、核心方法 iteratePitchingLoaders iterateNormalLoaders
console.time('cost');
